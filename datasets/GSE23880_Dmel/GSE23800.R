...
gse <- getGEO("GSE23880", GSEMatrix = T, AnnotGPL = T)[[1]]
...
tT$Entrez.ID[which(grepl("///", tT$Entrez.ID))] <- sapply(tT$Entrez.ID[which(grepl("///", tT$Entrez.ID))], FUN = function(x){return(strsplit(x, "///")[[1]][1])}, USE.NAMES = F)
